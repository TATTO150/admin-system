...
public function insertar(Request $request)
{
    ...
    // Asignar FEC_FINAL automáticamente (e.g., 6 meses después de FEC_INICIO)
    $fechaInicio = Carbon::parse($request->FEC_INICIO);
    $fechaFinal = $fechaInicio->addMonths(6); // Aquí sumas 6 meses a la fecha de inicio

    // Crear el proyecto usando el modelo
    $proyecto = new Proyectos();
    $proyecto->NOM_PROYECTO = $request->NOM_PROYECTO;
    $proyecto->FEC_INICIO = $request->FEC_INICIO;
    $proyecto->FEC_FINAL = $fechaFinal->toDateString(); // Asignar la fecha final
    $proyecto->DESC_PROYECTO = $request->DESC_PROYECTO;
    $proyecto->PRESUPUESTO_INICIO = $request->PRESUPUESTO_INICIO;
    $proyecto->ESTADO_PROYECTO = $request->input('ESTADO_PROYECTO', 'APERTURA');

    // Guardar el proyecto en la base de datos
    $proyecto->save();

    // Registrar en bitácora
    $this->bitacora->registrarEnBitacora(12, 'Se creó un nuevo proyecto', 'Insert');
...
public function destroy($COD_PROYECTO)
{
    ...
    try {
        ...
        $this->bitacora->registrarEnBitacora(12, 'Proyecto deshabilitado', 'destroy');//BITACORA
...
public function edit($COD_PROYECTO)
{
    ...
    // Verificar si el usuario tiene permiso para actualizar proyectos
    ...
    $proyectos = Proyectos::findOrFail($COD_PROYECTO); // Nota que ahora es $proyectos

    // Formatear fechas si es necesario
    $proyectos->FEC_FINAL = $proyectos->FEC_FINAL ? Carbon::parse($proyectos->FEC_FINAL)->format('Y-m-d') : null;

    // Obtener los estados del proyecto, excluyendo "INACTIVO"
    $estadosProyecto = EstadoProyecto::where('ESTADO_PROYECTO', '<>', 'INACTIVO')->get();

    $this->bitacora->registrarEnBitacora(12, 'Proyecto consultado para edición', 'Consulta'); //BITACORA
...
public function update(Request $request, $COD_PROYECTO)
{
    ...
    $proyecto = Proyectos::findOrFail($COD_PROYECTO);

    // Actualizar los datos del proyecto
    $proyecto->NOM_PROYECTO = $request->NOM_PROYECTO;
    $proyecto->FEC_INICIO = $request->FEC_INICIO;
    $proyecto->FEC_FINAL = $request->FEC_FINAL;
    $proyecto->DESC_PROYECTO = $request->DESC_PROYECTO;
    $proyecto->PRESUPUESTO_INICIO = $request->PRESUPUESTO_INICIO;
    ...
    $this->bitacora->registrarEnBitacora(12, 'Proyecto actualizado', 'Update'); //BITACORA
