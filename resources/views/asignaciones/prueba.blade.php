                ...
                @if($asignaciones->isNotEmpty())
                    @foreach ($asignaciones as $asignacion)
                        <tr data-cod-asignacion="{{ $asignacion->COD_ASIGNACION_EQUIPO }}">
                            <td>
                                <div class="dropdown">
                                    <button class="btn btn-secondary dropdown-toggle btn-sm" type="button" id="dropdownMenuButton{{ $asignacion->COD_ASIGNACION_EQUIPO }}" data-toggle="dropdown" aria-expanded="false">Acciones</button>
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $asignacion->COD_ASIGNACION_EQUIPO }}">
                                        ...
                                        @if($asignacion->COD_ESTADO_ASIGNACION == 1)
                                            <li><a class="dropdown-item" href="{{ route('asignaciones.gestionar', $asignacion->COD_ASIGNACION_EQUIPO) }}">GESTIONAR</a></li>
                                        @endif
                                        <li>
                                            <form action="{{ route('asignaciones.eliminar', $asignacion->COD_ASIGNACION_EQUIPO) }}" method="POST" class="d-inline eliminar-form" data-cod-asignacion="{{ $asignacion->COD_ASIGNACION_EQUIPO }}" data-estado="{{ $asignacion->COD_ESTADO_ASIGNACION }}">
                                                @csrf
                                                @method('PUT')
                                                <button type="button" class="dropdown-item eliminar-btn">ELIMINAR</button>
                                            </form>
                                        </li>
                                    </ul>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.eliminar-btn', function () {
                var codAsignacion = $(this).closest('.eliminar-form').data('cod-asignacion');
                var estadoAsignacion = $(this).closest('.eliminar-form').data('estado');

                if (estadoAsignacion == 2) {
                    Swal.fire({
                        title: '¿Estás seguro?',
                        text: "Esta asignación está finalizada. ¿Deseas eliminarla?",
                        icon: 'warning',
